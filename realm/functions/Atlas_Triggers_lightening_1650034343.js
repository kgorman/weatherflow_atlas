...
const fullDocument = changeEvent.fullDocument;
const collection = context.services.get("Cluster0").db("myFirstDatabase").collection("strikes");
var strikeFound = 0;

if (fullDocument && fullDocument.obs[0].lightning_strike_count_last_1hr > 0 && fullDocument.obs[0].lightning_strike_last_distance < 50 ) {
  ...
  const doc = collection.insertOne({ strike: strikeDocument });
  strikeFound = 1;
}
